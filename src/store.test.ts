...
test('test Store', async () => {
  let store = new Store(new JSONAPIAdapter({ host: 'https://www.zoocasa.com', namespace: 'services/api/v3', modelName: 'Listing' }), new JSONAPISerializer(testCreateModel));
  let result: any = await store.query({ filter: { status: 'available' }, page: { size: 2 }});
