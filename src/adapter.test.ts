...
test('test Adapter', async () => {
  ...
  expect((adapter as any).buildUrl({ filter: { bedrooms: 3 }, sort: '-price' })).toEqual('http://localhost:4200/api/v1/users?filter%5Bbedrooms%5D=3&sort=-price');
  ...
  expect(result.response.status).toEqual(200);
  expect((result.payload as { data: { id: string }}).data.id).toEqual('6909598');
});
